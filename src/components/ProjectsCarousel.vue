  ...
  <div class="relative">
    <div class="relative">
      <swiper :slides-per-view="1" :space-between="50" :pagination="{ clickable: true }" :loop="true"
        :modules="[Navigation, Pagination, Autoplay]" @swiper="onSwiper" class="w-full mb-10">
        <swiper-slide v-for="(project, index) in projects" :key="index" class="pb-5">
          <div class="flex flex-col md:flex-row rounded-lg shadow-medium overflow-hidden items-center">
            <div class="w-full md:w-1/2 h-64 relative overflow-hidden items">
              ...
              width="800" height="600" class="w-full h-full object-contain transition duration-300" />
              <div class="absolute top-3 left-3 bg-secondary text-black px-3 py-1 rounded">
                {{ project.category }}
              </div>
            </div>
            <div class="w-full md:w-1/2 p-5 flex flex-col justify-center">
              ...
              <div class="flex flex-wrap gap-2 mb-4">
                <span v-for="(tech, techIndex) in project.stack.slice(0, 10)" :key="techIndex"
                  class="bg-secondary text-black px-2 py-1 text-sm rounded">
                  {{ tech.name }}
                </span>
              </div>
    ...
    <div class="flex justify-between mt-4">
      <button @click="prevSlide" class="px-4 py-2 bg-zinc-800 text-white rounded-full hover:bg-primary transition"
        aria-label="Anterior proyecto">
        ←
      </button>
      <button @click="nextSlide" class="px-4 py-2 bg-zinc-800 text-white rounded-full hover:bg-primary transition"
...
const projects = ref([
  ...
  },
  {
    ...
    title: "Jobs&Wozniak",
    description:
      "Sitio de e-commerce ficticio de productos Apple donde los usuarios interactuar con un carrito, proceder al pago, iniciar y cerrar sesión en sus cuentas, y ver su información de usuario e historial de compras.",
    stack: [
      { name: "TypeScript" },
      { name: "Next.js" },
      { name: "Tailwind.css" },
  ...
  },
  {
    ...
    title: "Domenica",
    description:
      "Desarrollo de un gestor de reservas para los clientes de un restaurante ficticio llamado Domenica. Los usuarios pueden hacer y cancelar reservas, y la aplicación actualizará la base de datos en consecuencia.",
    stack: [
      { name: "TypeScript" },
      { name: "React" },
      { name: "Express" },
  ...
  },
  {
    ...
    title: "LatinoMedia",
    description:
      "Desarrollo de un catálogo de películas latinoamericanas. Los usuarios pueden agregar películas a través de un formulario, y la aplicación las incorporará a la base de datos y las mostrará en la página principal.",
    stack: [
      { name: "Javascript" },
      { name: "Bootstrap" },
      { name: "Express" },
